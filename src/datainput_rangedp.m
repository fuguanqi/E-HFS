...
function y=datainput_rangedp(xx,prob) %objective function
...
for j=1:n_S
    ...
    if j==n_S
        ...
        rate=x(prob.int_dim+(j-1)*n+1:prob.int_dim+j*n);
        [stage_C,stage_EP,stage_ETI]=timingByRangeDP(job_seq,assign,pr_speed,A(:,j),j,rate,prob);
    end
